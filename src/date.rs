...
impl SerializableDate for Date {
    ...
    fn serialize_4_bytes(&self) -> Result<[u8; 4]> {
        let reference_date = Utc.with_ymd_and_hms(2001, 1, 1, 0, 0, 0)
            .single()
            .unwrap();
...
mod tests {
    ...
    fn test_2_byte_dates() {
        ...
        let expected_max_date = Date::from_datetime(Utc.with_ymd_and_hms(2150, 12, 31, 0, 0, 0).unwrap());
    ...
    fn test_4_byte_dates() {
        ...
        let expected_max_date = Date::from_datetime(Utc.with_ymd_and_hms(2137, 2, 7, 6, 28, 15).unwrap());
        assert_eq!(deserialized_max, expected_max_date);
    }
    ...
    fn test_6_byte_dates() {
        // Base date serialization and deserialization
        let base_date = Date::from_datetime(Utc.with_ymd_and_hms(2023, 6, 20, 12, 34, 56).unwrap().with_nanosecond(789_000_000).unwrap());
        ...
        let expected_max_date = Date::from_datetime(Utc.with_ymd_and_hms(9999, 12, 31, 23, 59, 59).unwrap().with_nanosecond(999_000_000).unwrap());
        assert_eq!(deserialized_max, expected_max_date);
